...
psql -v ON_ERROR_STOP=1 -U $DATABASE_USER -d $DATABASE_NAME -h $DATABASE_HOST <<-EOSQL
  ...
  CREATE VIEW tickets_view AS SELECT id, client_name AS "clientName", client_phone AS "clientPhone", client_address AS "clientAddress", description, is_finished AS "isFinished" FROM tickets;
